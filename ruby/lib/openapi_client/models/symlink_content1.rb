  ...
  class SymlinkContent1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SymlinkContent1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SymlinkContent1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @submodule_git_url.nil?
        invalid_properties.push('invalid value for "submodule_git_url", submodule_git_url cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @submodule_git_url.nil?
    ...
    def hash
      [type, submodule_git_url, size, name, path, sha, url, git_url, html_url, download_url, _links].hash
    end
