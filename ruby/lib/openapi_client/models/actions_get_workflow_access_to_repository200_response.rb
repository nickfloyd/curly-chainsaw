...
module OpenapiClient
  class ActionsGetWorkflowAccessToRepository200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ActionsGetWorkflowAccessToRepository200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ActionsGetWorkflowAccessToRepository200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      access_level_validator = EnumAttributeValidator.new('String', ["none", "user", "organization", "enterprise"])
    ...
    def access_level=(access_level)
      validator = EnumAttributeValidator.new('String', ["none", "user", "organization", "enterprise"])
