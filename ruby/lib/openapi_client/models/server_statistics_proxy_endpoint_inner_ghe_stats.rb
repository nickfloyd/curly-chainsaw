...
module OpenapiClient
  class ServerStatisticsProxyEndpointInnerGheStats
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ServerStatisticsProxyEndpointInnerGheStats` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ServerStatisticsProxyEndpointInnerGheStats`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'gists')
        self.gists = attributes[:'gists']
      end
      ...
      if attributes.key?(:'pulls')
        self.pulls = attributes[:'pulls']
      end
    ...
    def hash
      [comments, gists, hooks, issues, milestones, orgs, pages, pulls, repos, users].hash
    end
