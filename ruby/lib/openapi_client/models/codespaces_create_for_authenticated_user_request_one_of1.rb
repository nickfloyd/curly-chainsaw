  ...
  class CodespacesCreateForAuthenticatedUserRequestOneOf1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CodespacesCreateForAuthenticatedUserRequestOneOf1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CodespacesCreateForAuthenticatedUserRequestOneOf1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @pull_request.nil?
        invalid_properties.push('invalid value for "pull_request", pull_request cannot be nil.')
      end
    ...
    def hash
      [pull_request, location, machine, devcontainer_path, working_directory, idle_timeout_minutes].hash
    end
