...
module OpenapiClient
  ...
  class DiffEntry
    ...
    attr_accessor :previous_filename
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DiffEntry` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DiffEntry`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'previous_filename')
        self.previous_filename = attributes[:'previous_filename']
      end
    end
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["added", "removed", "modified", "renamed", "copied", "changed", "unchanged"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["added", "removed", "modified", "renamed", "copied", "changed", "unchanged"])
    ...
    def hash
      [sha, filename, status, additions, deletions, changes, blob_url, raw_url, contents_url, patch, previous_filename].hash
    end
