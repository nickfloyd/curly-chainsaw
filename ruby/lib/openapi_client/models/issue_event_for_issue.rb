  ...
  class IssueEventForIssue
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::IssueEventForIssue` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::IssueEventForIssue`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @performed_via_github_app.nil?
        invalid_properties.push('invalid value for "performed_via_github_app", performed_via_github_app cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @performed_via_github_app.nil?
    ...
    def hash
      [id, node_id, url, actor, event, commit_id, commit_url, created_at, performed_via_github_app, label, assignee, assigner, milestone, rename, review_requester, requested_team, requested_reviewer, dismissed_review, lock_reason, project_card].hash
    end
