  ...
  class PullRequestSimpleLinks
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PullRequestSimpleLinks` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PullRequestSimpleLinks`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @review_comment.nil?
        invalid_properties.push('invalid value for "review_comment", review_comment cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @review_comment.nil?
    ...
    def hash
      [comments, commits, statuses, html, issue, review_comments, review_comment, _self].hash
    end
