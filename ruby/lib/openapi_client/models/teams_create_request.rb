  ...
  class TeamsCreateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TeamsCreateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TeamsCreateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'repo_names')
        if (value = attributes[:'repo_names']).is_a?(Array)
          self.repo_names = value
        end
      end
    ...
    def valid?
      ...
      privacy_validator = EnumAttributeValidator.new('String', ["secret", "closed"])
      return false unless privacy_validator.valid?(@privacy)
      permission_validator = EnumAttributeValidator.new('String', ["pull", "push"])
    ...
    def privacy=(privacy)
      validator = EnumAttributeValidator.new('String', ["secret", "closed"])
    ...
    def permission=(permission)
      validator = EnumAttributeValidator.new('String', ["pull", "push"])
    ...
    def hash
      [name, description, maintainers, repo_names, privacy, permission, parent_team_id].hash
    end
