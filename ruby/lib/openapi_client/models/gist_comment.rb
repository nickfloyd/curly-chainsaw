  ...
  class GistComment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GistComment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GistComment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @body.to_s.length > 65535
        invalid_properties.push('invalid value for "body", the character length must be smaller than or equal to 65535.')
      end
    ...
    def valid?
      ...
      return false if @body.to_s.length > 65535
    ...
    def body=(body)
      ...
      if body.to_s.length > 65535
        fail ArgumentError, 'invalid value for "body", the character length must be smaller than or equal to 65535.'
      end
    ...
    def hash
      [id, node_id, url, body, user, created_at, updated_at, author_association].hash
    end
